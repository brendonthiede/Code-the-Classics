"""
Substitute Soccer - browser-native version
==========================================
This file is a port of soccer-master/soccer.py from Pygame Zero (pgzero) to
pure Pygame with an asyncio-based game loop, making it compatible with pygbag
(https://pygame-web.github.io/).

Run in dev mode (hot-reload in the browser):
    pip install pygbag
    pygbag soccer-web/

Build static WASM for deployment:
    pygbag --build soccer-web/

Controls  Player 1: Arrow keys + Space   |  Player 2: WASD + Left-Shift
"""
...
# ---------------------------------------------------------------------------
# Pygame Zero compatibility shim
# Provides drop-in replacements for the pgzero APIs used by the game so that
# the original game logic below requires no changes.
# ---------------------------------------------------------------------------

# --- Key constants (mirrors pgzero.builtins.keys) --------------------------


class keys:  # noqa: N801 – intentional lower-case to match pgzero API
    UP = pygame.K_UP
    DOWN = pygame.K_DOWN
    LEFT = pygame.K_LEFT
...
# --- Keyboard state (mirrors pgzero's `keyboard` object) --------------------


class _Keyboard:
    """Dict-like: keyboard[pygame.K_UP] → True/False each frame."""
...
def _load_image(name: str) -> pygame.Surface:
    """Load *name* from images/, caching the result."""
    if name not in _image_cache:
        for ext in (".png", ".jpg", ".gif"):
            ...
            _image_cache[name] = pygame.Surface((1, 1), pygame.SRCALPHA)
    return _image_cache[name]


# --- screen.draw helper (subset used by the game) ----------------------------
...
class _DrawHelper:
    ...
    def text(self, text, center=None, pos=None, color=(255, 255, 255), fontsize=20):
        if not hasattr(self, "_font_cache"):
            self._font_cache = {}
        font = self._font_cache.get(fontsize)
        if font is None:
            font = pygame.font.Font(None, fontsize)
            self._font_cache[fontsize] = font
...
# --- screen object (mirrors pgzero's `screen`) --------------------------------


class _Screen:
    def __init__(self):
        self.surface: pygame.Surface = None
        self.draw: _DrawHelper = None

    def _init(self, surface: pygame.Surface):
        self.surface = surface
        self.draw = _DrawHelper(surface)

    def blit(self, image_or_name, pos):
        surf = _load_image(image_or_name) if isinstance(image_or_name, str) else image_or_name
        self.surface.blit(surf, (int(pos[0]), int(pos[1])))


screen = _Screen()


# --- Sound proxies (mirrors pgzero's `sounds`) --------------------------------
...
class _Sounds:
    """Attribute-access loader: sounds.kick0 → _SoundProxy('kick0')."""

    def __getattr__(self, name: str) -> _SoundProxy:
        proxy = _SoundProxy(name)
...
# --- Actor (replaces pgzero's Actor) -----------------------------------------


class Actor:
    """
    Minimal drop-in for pgzero.actor.Actor.

    pos  – position of the anchor point on screen.
    anchor – None → ('center','center'); tuple of strings or pixel offsets.
    """

    def __init__(self, image_name: str, pos=(0, 0), anchor=None):
        self._image_name = image_name
        self._surface: pygame.Surface = None
        self._anchor = ("center", "center") if anchor is None else anchor
        self._x = float(pos[0])
        self._y = float(pos[1])
    ...
    @property
    def width(self) -> int:
        return self._get_surface().get_width()

    @property
    def height(self) -> int:
        return self._get_surface().get_height()
    ...
    def _topleft(self):
        ...
        w, h = surf.get_size()
        ax, ay = self._anchor
        ox = 0 if ax == "left" else w if ax == "right" else w // 2 if ax == "center" else int(ax)
        oy = 0 if ay == "top" else h if ay == "bottom" else h // 2 if ay == "center" else int(ay)
        return (int(self._x - ox), int(self._y - oy))

    def draw(self):
        screen.surface.blit(self._get_surface(), self._topleft())


# ---------------------------------------------------------------------------
# End of pgzero compatibility shim
# ---------------------------------------------------------------------------

# (imports handled by shim above)
...
def init_audio():
    ...
    # Emscripten/pygbag: audio is managed by the browser; attempt a single init.
    # Use a larger buffer (4096) and explicit sample rate to reduce crackling and
    # suppress the SDL_mixer "prefer OGG over MP3" warning in the browser console.
    if "emscripten" in sys.platform:
        try:
            pygame.mixer.init(44100, -16, 2, 4096)
            AUDIO_AVAILABLE = True
            AUDIO_DRIVER = os.environ.get("SDL_AUDIODRIVER", "emscripten")
        except Exception:
            AUDIO_AVAILABLE = False
        return
    ...
    if dummy_worked:
        ...
        if os.path.exists("/mnt/wslg/PulseServer"):
            print(
                "Hint (WSL): install libpulse0 (for libpulse-simple.so.0). WSLg already provides the Pulse server.", file=sys.stderr
            )
        else:
            print(
                "Hint (Linux): install audio backend libs (commonly libasound.so.2 and libpulse-simple.so.0) and ensure an audio device is available.",
                file=sys.stderr,
            )
        return
    ...
    for d, err in errors[:6]:
        print("  tried {0}: {1}".format(d, err), file=sys.stderr)
    print(
        "Hint (Linux): install ALSA/Pulse libs (e.g. libasound.so.2, libpulse-simple.so.0) or run with audio access.",
        file=sys.stderr,
    )
    ...
    def update(self):
        ...
        if self.owner:
            ...
            if team.human():
                ...
                do_shoot = (
                    self.timer <= 0 and target and cost(target.vpos, self.owner.team) < cost(self.owner.vpos, self.owner.team)
                )
            ...
            if do_shoot:
                ...
                if target:
                    ...
                    target = min([p for p in game.players if p.team == self.owner.team], key=dist_key(self.vpos + (vec * 250)))
...
class Game:
        ...
        for team_num in range(2):
            ...
            if team_obj.human() and team_obj.controls.shoot():
                ...
                self.teams[team_num].active_control_player = min(
                    [p for p in game.players if p.team == team_num], key=dist_key_weighted
                )
    ...
    def draw(self):
        ...
        objects = sorted([self.ball] + self.players, key=lambda obj: obj.vpos.y)
...
# ---------------------------------------------------------------------------
# pygbag requires the game to use asyncio and yield control each frame via
# `await asyncio.sleep(0)`.  The update() and draw() functions defined above
# are called exactly as pgzero would have called them.


async def main():
    # Pre-initialise the mixer before pygame.init() so SDL_mixer picks up our
    # preferred settings (44100 Hz, 16-bit stereo, 4096-sample buffer) rather
    # than its defaults.  This also suppresses the "prefer OGG over MP3" console
    # warning in browsers by ensuring OGG support is configured up front.
    pygame.mixer.pre_init(44100, -16, 2, 4096)
    pygame.init()
    display_surface = pygame.display.set_mode((WIDTH, HEIGHT))
    pygame.display.set_caption(TITLE)
    screen._init(display_surface)

    # Mirror the original soccer.py startup sequence
    init_audio()
    ...
    while True:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                # Stop all audio cleanly before exiting to prevent the
                # glitchy/crackling noise heard when the browser tab is closed.
                try:
